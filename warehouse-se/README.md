...
# example
$ docker build -t warehouse-se -f deployment/dockerfile .
```

### run docker container after build the image
```bash
# example run http serve:
$ docker run -i --name warehouse-se -p 8081:8081 -t warehouse-se
...
mkdir $HOME/go/pkg
mkdir $HOME/go/bin
mkdir -p $HOME/go/src/warehouse-se
chmod -R 775 $HOME/go
cd $HOME/go/src/warehouse-se
...
# run command :
cd $HOME/go/src/warehouse-se
git clone -b development https://warehouse-se .
cd $HOME/go/src/warehouse-se
...
```bash
docker build -t warehouse-se -f deployment/dockerfile-local .
docker run -i --name warehouse-se -p 8081:8081 -v $PWD:/go/src/warehouse-se/warehouse-se warehouse-se
```
