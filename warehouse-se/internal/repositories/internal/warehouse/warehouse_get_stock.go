...
func (r warehouse) GetStock(ctx context.Context, warehouseID string) ([]entity.ProductStock, error) {
	query := `
	SELECT jsonb_agg(
        jsonb_build_object(
            'id', s.id,
	...
	err := r.db.QueryRow(ctx, &b, query, warehouseID)
	if err != nil {
	...
	if len(b) == 0 || string(b) == "null" {
		return []entity.ProductStock{}, nil // return empty slice if no data
	}

	if err := json.Unmarshal(b, &stock); err != nil {
		logger.Info(err.Error())
		return nil, errors.Wrap(err, "failed to unmarshal byte to ProductStock")
	}
