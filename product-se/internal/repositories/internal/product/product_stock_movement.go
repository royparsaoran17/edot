...
func (r product) GetStockMovement(ctx context.Context) ([]entity.StockMovement, error) {
	query := `
	SELECT jsonb_agg(
        jsonb_build_object(
            'id', s.id,
            ...
            'from_warehouse', (
                ...
                FROM warehouses fw
                WHERE s.from_warehouse_id = fw.id AND fw.deleted_at IS NULL
            ),
            ...
            'to_warehouse', (
                ...
                FROM warehouses tw
                WHERE s.to_warehouse_id = tw.id AND tw.deleted_at IS NULL
            ),
            'notes', s.notes,
            'quantity', s.quantity,
