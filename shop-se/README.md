...
# example
$ docker build -t shop-se -f deployment/dockerfile .
```

### run docker container after build the image
```bash
# example run http serve:
$ docker run -i --name shop-se -p 8081:8081 -t shop-se
...
mkdir $HOME/go/pkg
mkdir $HOME/go/bin
mkdir -p $HOME/go/src/shop-se
chmod -R 775 $HOME/go
cd $HOME/go/src/shop-se
...
# run command :
cd $HOME/go/src/shop-se
git clone -b development https://shop-se .
cd $HOME/go/src/shop-se
...
```bash
docker build -t shop-se -f deployment/dockerfile-local .
docker run -i --name shop-se -p 8081:8081 -v $PWD:/go/src/shop-se/shop-se shop-se
```
