...
# example
$ docker build -t order-se -f deployment/dockerfile .
```

### run docker container after build the image
```bash
# example run http serve:
$ docker run -i --name order-se -p 8081:8081 -t order-se
...
mkdir $HOME/go/pkg
mkdir $HOME/go/bin
mkdir -p $HOME/go/src/order-se
chmod -R 775 $HOME/go
cd $HOME/go/src/order-se
...
# run command :
cd $HOME/go/src/order-se
git clone -b development https://order-se .
cd $HOME/go/src/order-se
...
```bash
docker build -t order-se -f deployment/dockerfile-local .
docker run -i --name order-se -p 8081:8081 -v $PWD:/go/src/order-se/order-se order-se
```
